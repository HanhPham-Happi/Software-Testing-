...
public class CreateAccountScreen {
	...
	public static void createAccount(WebDriver driver, String name, String email, String password, String passwordConfirm, String phone, String msgXpath, String expectedMsg) throws Exception {
		Helper.sendKeys(driver, NAME_TXT_XPATH, name);
		Helper.sendKeys(driver, EMAIL_TXT_XPATH, email);
		Helper.sendKeys(driver, PWD_TXT_XPATH, password);
		Helper.sendKeys(driver, PWD_CFM_TXT_XPATH, passwordConfirm);
		Helper.sendKeys(driver, PHONE_TXT_XPATH, phone);
		Helper.clickItem(driver, CREATE_ACC_BTN_XPATH);
		TimeUnit.SECONDS.sleep(3);
		if (expectedMsg == SUCCESS_MSG) {
			Helper.assertTextValue(driver, msgXpath, expectedMsg.replace(Constant.EMAIL_VALUE, email));
		}
		else {
			Helper.assertTextValue(driver, msgXpath, expectedMsg);
		}
	}
}
